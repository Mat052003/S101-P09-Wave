...
export default function AdminDashboard() {
  ...
  if (loading) {
    return (
      <div className="p-8 flex items-center justify-center min-h-96">
        <p className="text-[#0B1F2D]/40">{t("loading")}</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 md:p-8 max-w-6xl mx-auto space-y-8">
      ...
      {/* Header */}
      <div>
        <p className="text-[10px] font-semibold uppercase tracking-[0.35em] text-[#C9A87C] mb-1">{t("hostPanel")}</p>
        <h1 className="font-display text-3xl font-bold text-[#0B1F2D]">Dashboard</h1>
        <p className="text-[#0B1F2D]/40 text-sm mt-1">{t("subtitle")}</p>
      </div>

      {/* KPIs */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: t("publishedHotels"),  value: stats.totalHotels,          icon: "🏨", color: "bg-[#0B1F2D]",   text: "text-white" },
          { label: t("totalReservations"),   value: stats.totalReservations,    icon: "📋", color: "bg-[#C9A87C]",   text: "text-[#0B1F2D]" },
      ...
      <div className="grid md:grid-cols-2 gap-4">
        ...
        <div className="bg-white rounded-2xl border-2 border-[#0B1F2D]/10 p-6">
          <p className="text-xs font-bold uppercase tracking-wider text-[#0B1F2D]/40 mb-1">{t("avgPrice")}</p>
            ...
            {stats.hotelStats.map((h) => (
              <div key={h.name} className="flex items-center gap-4 p-3 bg-[#FAF6F0] rounded-xl">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-[#0B1F2D]/40">{h.reservations} {t("reservations")} · ${h.revenue.toLocaleString()} USD</p>
                </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
        {[
          { label: t("myHotels"),    href: "/admin/hotels",      icon: "🏨" },
          { label: t("addHotel"),      href: "/admin/hotels/new",  icon: "➕" },
