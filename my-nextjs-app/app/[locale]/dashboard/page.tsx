...
export default async function DashboardPage({
  ...
  const statusLabel: Record<string, { label: string; color: string }> = {
    PENDING:   { label: t("status.PENDING"),  color: "bg-[#B4B8AB]/28 text-[#153243] border border-[#153243]/20" },
    CONFIRMED: { label: t("status.CONFIRMED"), color: "bg-[#F4F9E9] text-[#153243] border border-[#153243]/20"   },
    CANCELLED: { label: t("status.CANCELLED"),  color: "bg-[#EEF0EB] text-[#153243] border border-[#153243]/25"   },
  };
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-6 py-12 space-y-9">
        <section className="panel rounded-3xl border border-[#153243]/16 bg-[#F4F9E9]/95 p-8">
          <div className="flex items-center gap-5">
            ...
            <div>
              <h1 className="font-display text-3xl font-semibold text-[#153243]">{user?.name ?? t("user")}</h1>
          ...
          <div className="mt-7 grid grid-cols-1 sm:grid-cols-3 gap-4 pt-7 border-t border-[#153243]/12">
            <div className="text-center rounded-2xl bg-[#EEF0EB] border border-[#153243]/16 p-4">
              ...
              <p className="text-xs text-[#284B63]/85 uppercase tracking-[0.16em] mt-1">{t("reservations")}</p>
            </div>
            <div className="text-center rounded-2xl bg-[#EEF0EB] border border-[#153243]/16 p-4">
              <p className="text-2xl font-black text-[#153243]">
                {reservations.filter((r) => r.status === "CONFIRMED").length}
              </p>
              <p className="text-xs text-[#284B63]/85 uppercase tracking-[0.16em] mt-1">{t("confirmed")}</p>
            </div>
            <div className="text-center rounded-2xl bg-[#EEF0EB] border border-[#153243]/16 p-4">
              <p className="text-2xl font-black text-[#153243]">
                ${reservations.reduce((sum, r) => sum + r.totalPrice, 0).toFixed(0)}
              </p>
              <p className="text-xs text-[#284B63]/85 uppercase tracking-[0.16em] mt-1">{t("totalSpent")}</p>
            </div>
          </div>
        </section>

        <section>
          <h2 className="font-display text-3xl font-semibold text-[#153243] mb-4">{t("reservationHistory")}</h2>

          {reservations.length === 0 ? (
            <div className="panel rounded-3xl border border-[#153243]/16 bg-[#F4F9E9]/95 p-12 text-center">
              <p className="text-[#153243] font-semibold">{t("noReservations")}</p>
              <Link
                href="/"
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-[#284B63]/75 mt-0.5">
                          ...
                          {new Date(res.checkOut).toLocaleDateString("es-CL")} · {t("nights", { count: nights })}
                        </p>
                      </div>
                    </div>
