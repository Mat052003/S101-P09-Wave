...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-[calc(100vh-4rem)] bg-[#FAF6F0]">
      ...
      {/* Sidebar */}
      <aside className={`${open ? "w-56" : "w-14"} shrink-0 bg-[#0B1F2D] flex flex-col transition-all duration-300 sticky top-16 h-[calc(100vh-4rem)] overflow-hidden`}>

        {/* Toggle */}
        <button
          onClick={() => setOpen(!open)}
          className="flex items-center justify-center h-11 w-full border-b border-white/10 hover:bg-white/5 transition-colors shrink-0">
          <span className="text-white/50 text-sm font-bold">{open ? "«" : "»"}</span>
        </button>
              ...
              return (
                <Link key={item.href} href={item.href}
                  className={`flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all ${
                    active ? "bg-[#C9A87C] text-[#0B1F2D]" : "text-white/70 hover:bg-white/10 hover:text-white"
                  }`}>
                  <div className={`w-1.5 h-1.5 rounded-full shrink-0 ${active ? "bg-[#0B1F2D]" : "bg-white/40"}`} />
              ...
              <div key={item.label}>
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all ${
                    hasActiveChild ? "text-[#C9A87C]" : "text-white/70 hover:bg-white/10 hover:text-white"
                  }`}>
                  <div className={`w-1.5 h-1.5 rounded-full shrink-0 ${hasActiveChild ? "bg-[#C9A87C]" : "bg-white/40"}`} />
                  {open && (
                    <>
                      <span className="text-sm font-semibold flex-1 text-left truncate">{item.label}</span>
                      <span className="text-xs text-white/30">{isGroupExpanded ? "▾" : "▸"}</span>
                    </>
                  )}
                </button>
                      ...
                      return (
                        <Link key={child.href} href={child.href}
                          className={`block px-3 py-2 rounded-xl text-xs font-medium transition-all ${
                            active ? "bg-[#C9A87C]/20 text-[#C9A87C]" : "text-white/50 hover:bg-white/10 hover:text-white"
                          }`}>
                          {child.label}
                        </Link>
                      );
